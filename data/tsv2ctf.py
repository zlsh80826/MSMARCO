...
def tsv_iter(line, vocab, chars, known, is_test=False, misc={}):
    unk_w = vocab[unk]
    unk_c = chars[unk]
    ...
    ccids = [[chars.get(c, unk_c) for c in t][:word_size] for t in ctokens]
...
def tsv_to_ctf(f, g, vocab, chars, known, is_test):
    ...
    for tsv in tsvs:
        with open('%s.tsv' % tsv, 'r', encoding='utf-8') as f:
            with open('%s.ctf' % tsv, 'w', encoding='utf-8') as g:
                tsv_to_ctf(f, g, vocab, chars, known, tsv == 'test' or tsv == 'test_public')
